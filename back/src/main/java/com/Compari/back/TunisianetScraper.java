...
import com.Compari.back.service.ProductsImgsService;
import com.Compari.back.service.ProductsService;
...
public class TunisianetScraper implements ApplicationRunner {
    ...
    private List<ProductsImgs> extractImages(Document productDoc, String productName) {
        ...
        // Sanitize the product name for folder path
        String sanitizedProductName = sanitizeFileName(productName);
        Path imgFolderPath = Paths.get("C:\\Users\\ASUS\\Desktop\\A\\Website\\COMPARI\\Front_User\\src\\assets\\img", sanitizedProductName);
        ...
        if (!Files.exists(imgFolderPath)) {
            try {
                ...
                return imgs; // Return an empty list if directory creation fails
            }
        }
        ...
        for (Element imgElement : imgElements) {
            ...
            imgName = sanitizeFileName(imgName); // Sanitize the image name
            ...
            try (InputStream in = new URL(imgUrl).openStream()) {
                ...
                System.err.println("Failed to save image " + imgName + ": " + e.getMessage());
            }
        }
        return imgs;
    }
